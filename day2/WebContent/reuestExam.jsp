...
<body>
request 내장객체
내장 객체(implicit object)는 개발자가 객체를 생성하지 않고 바로 사용할 수 있는 객체로 내부객체 또는 기본객체
자동으로 객체가 생성된다. 간단하게 쉽게 활용할 수 있다. 
대표적인 내장 객체 - response, session, config, page, exception....

JSP에서 가장 많이 사용되는 내장객체, 웹 브라우저에서 서버로 연결한 후에 요청 정보를 전달한다.
- 클라이언트 관련정보

getRemoteAddr()	클리이언트 IP주소 리턴
...
getRemoteAddr()	클리이언트 IP주소 리턴 :<%=request.getRemoteAddr() %><br>
